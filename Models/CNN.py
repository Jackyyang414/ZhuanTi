...
a=0
print("start.......")
print("Start loading RadarValue......")
...
for x in range(len(RadarPath)):
    ...
    for RadarFile in RadarList:
        ...
        RadarHM=RadarFile[20:24]
        RadarTime=RadarDate+RadarHM
        if RadarTime in diff:
           np.insert(Label,RadarTimeList.index(RadarTime),[1,0,0],0)
...
#卷積層1
model.add(Conv2D(filters=16, kernel_size=(6,6), padding='same', input_shape=(21,149,149), activation='relu'))
#池化屠1
...
train_history=model.fit(x=train_X, y=train_y,validation_split=0.2, epochs=1000,batch_size=32,verbose=2)

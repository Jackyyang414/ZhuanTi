...
X=RList.reshape(Quantity,21,149,149).astype('float32')
Y=Label
print(X.shape)
print(Y.shape)
...
#卷積層
# 1
model.add(Conv3D(32, kernel_size=(3,3,1), padding='same', input_shape=(period,21,149,149), activation=relu))
#池化屠1
model.add(MaxPooling3D(pool_size=(2,2,1)))
#卷積層2
model.add(Conv3D(64,kernel_size=(3,3,1),padding='same',activation=relu))
#池化屠2
model.add(MaxPooling3D(pool_size=(2, 2, 1), padding='same'))
#Conv3
model.add(Conv3D(128, kernel_size=(3, 3, 1), padding='same', activation=relu))
...
model.add(Dense(128, input_dim=64,
                ...
                activity_regularizer=regularizers.l1(0.08),
                activation='relu'))
'''
...
train_history=model.fit(x=train_X, y=train_y,validation_split=0.2, epochs=EpochSize,batch_size=32,verbose=2)
